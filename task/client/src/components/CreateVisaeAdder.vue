...
const AgencyVisaEmployeeServiceSelectList = async (value) => {
  store.commit("setLoading", true, { root: true });
  try {
    const res = await AgencyVisaEmployeeService.GetAsSelectList(value);
...
// watch([agencyId], () => {
//   items.value.agencyId = agencyId;
//   AgencyVisaEmployeeServiceSelectList(agencyId);
// });

const isShowModalOne = ref(true);
const ModalOne = () => {
  isShowModalOne.value = !isShowModalOne.value;
};

const Ministry_Table = () => {
  if (
    ...
    store.dispatch("Ministry_Table", items.value);
    ModalOne();
    items.value = {};
...
<template>
  <div class="modal" v-show="isShowModalOne">
    <div class="">
      <div
        class="relative z-10"
